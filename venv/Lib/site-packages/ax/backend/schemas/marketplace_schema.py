...
async def check_form_integrity(db_session, ax_form):
    with ax_model.scoped_session(
            "marketplace_schema -> check_form_integrity") as db_session:
        ...
        for perm in ax_form.permissions:
            ...
            if str(perm.role_guid) not in roles_guids \
                ...
                must_be_deleted.append(perm.guid)

        error_count = len(must_be_deleted)
        logger.warning(f"Found intergrity {error_count} erros")
        db_session.query(AxRoleFieldPermission).filter(
            AxRoleFieldPermission.guid.in_(must_be_deleted)).delete(
...
async def create_role_field_perm(db_session, ax_form, perm):
    ...
    with ax_model.try_catch(db_session, err) as db_session:
        try:
            ...
            print(message)
            perm_str = str(new_perm)
            logger.error(
                f'create_role_field_perm -> Foreign key error! {perm_str}')
