...
def init_routes(sanic_app, pages_path=None, ssl_enabled=False):  # pylint: disable=unused-variable
    ...
    try:
        ...
        async def test(request):  # pylint: disable=unused-variable
            ...
            ret_str = ""
            error_counter = 0
            with ax_model.scoped_session("Integrity check") as db_session:

                # For all forms
                all_forms = db_session.query(AxForm).all()
                for ax_form in all_forms:
                    # Collect roles, states, fields guids
                    roles_guids = [str(x.guid) for x in ax_form.roles]
                    states_guids = [str(x.guid) for x in ax_form.states]
                    fields_guids = [str(x.guid) for x in ax_form.fields]

                    for perm in ax_form.permissions:
                        error_counter = 0
                        if str(perm.role_guid) not in roles_guids \
                                or str(perm.role_guid) not in states_guids \
                                or str(perm.role_guid) not in fields_guids:
                            error_counter += 1

                    # # Select all permissions with for this form,
                    # sql = "SELECT * FROM _ax_role_field_permissions WHERE form_guid=:form_guid"
                    # variables = {
                    #     "form_guid": str(ax_form.guid)
                    # }
                    # all_perms = db_session.execute(sql, variables).fetchall()

                    # # For each perm - check if role, state, field exists
                    # for perm in all_perms:
                    #     error_counter_1 = False
                    #     if perm["role_guid"] not in roles_guids:
                    #         error_counter_1 += 1
                    #     if perm["state_guid"] not in states_guids:
                    #         error_counter_1 += 1
                    #     if perm["field_guid"] not in fields_guids:
                    #         error_counter_1 += 1

            return response.text(f"err1 = {error_counter}")
